...
class DartSassCompilerTest extends AnyFunSpec with Matchers {
  ...
  val compiler = DartSassCompiler(compilerSettings)

  describe("The DartSassCompiler") {
    describe("using well formed scss input") {
      describe("without includes") {
        ...
        val compilationResults = compiler.compile(Seq(input), input.getParent, testDir)(input)
    ...
    describe("using broken scss input") {
      ...
      val compilationResult = compiler.compile(Seq(input), input.getParent, testDir)(input)
      ...
      describe("should throw an exception") {
        ...
        it("reporting an error on line 2 column 15") {
          compilationResult match {
            case Left(exception) =>
              val message = exception.logMessages.head
              message.location.get.line mustBe 2
              message.location.get.column mustBe 15
            case _ => fail
          }
        }
      }
