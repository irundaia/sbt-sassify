    ...
    describe("using well formed scss input") {
      describe("without includes") {
        it("should compile") {
          ...
          val processOutput = new SassCompiler(CompilerSettings(CssStyle.Minified, generateSourceMaps = true))
    ...
    describe("using broken scss input") {
      it("should throw an exception") {
        ...
        val exception = the [SassCompilerException] thrownBy
          new SassCompiler(CompilerSettings(CssStyle.Minified, generateSourceMaps = true))
            .doCompile(input, outputMinified, mapMinified, "", "")
